...
export function ProfileForm({ user }: { user: User }) {
  ...
  function handleSubmit(formData: FormData) {
    // Override avatarUrl with the uploaded one
  ...
  return (
    <form action={handleSubmit} className="rounded-xl border border-[var(--color-secondary-4)] bg-[var(--color-secondary-2)] p-6">
      ...
      <div className="grid gap-4 sm:grid-cols-2">
        ...
        <FieldGroup label="Display Name" name="displayName" defaultValue={user.displayName} />
        <FieldGroup label="Job Title" name="jobTitle" defaultValue={user.jobTitle ?? ""} />
        <FieldGroup label="Company" name="company" defaultValue={user.company ?? ""} />
        <FieldGroup label="Email" name="email" type="email" defaultValue={user.email ?? ""} />
        <FieldGroup label="Phone" name="phone" type="tel" defaultValue={user.phone ?? ""} />
      ...
      <div className="mt-6 border-t border-[var(--color-secondary-4)] pt-6">
        ...
        <div className="space-y-4">
          <FieldGroup
            label="Redirect URL (overrides card — all traffic redirects here)"
            name="redirectUrl"
